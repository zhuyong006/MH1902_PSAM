 ...
 * File Name            : usbh_hid_core.c
 * Author               : Megahuntmicro
 * Version              : V1.0.0
 * Date                 : 21-October-2014
 * Description          : This file is the HID Layer Handlers for USB Host HID class.
 *****************************************************************************/
                                   ...
                                   void   *phost)
{
  ...
  case HID_POLL:
    if((( HCD_GetCurrentFrame(pdev) - HID_Machine.timer) >= HID_Machine.poll) || \
        ((HCD_GetCurrentFrame(pdev) - HID_Machine.timer) < 0))
    {
        HID_Machine.state = HID_GET_DATA;
    }
    else if(HCD_GetURB_State(pdev , HID_Machine.hc_num_in) == URB_DONE)
    {
		...
		{
			start_toggle = 0;
			printf("%02x %02x %02x %02x %02x %02x\n",HID_Machine.buff[0],HID_Machine.buff[1],\
													 HID_Machine.buff[2],HID_Machine.buff[3],\
													 HID_Machine.buff[4],HID_Machine.buff[5]);
