...
static  SCI_TypeDef *  const aSCI_BaseTable[3] = {(SCI_TypeDef *)SCI0_BASE};
static  const uint8_t        aSCI_CG_Table[3] = {(uint8_t)SCI0_CG_BIT, (uint8_t)SCI1_CG_BIT, (uint8_t)SCI2_CG_BIT};
volatile struct emv_queue  scs_queue[SLOT_COUNT]; /* datas buffer */
...
//                //printf("slot %d deviation percent %+1.2f%%!\n", i, deviation);
...
#if (0 != EMV_UNIT_TEST)
static uint8_t gu8Step;
uint32_t u32TranStep = 2;
uint32_t u32ErrorStep = 0;
#endif
...
int emv_hard_cold_reset( struct emv_core *pdev )
{
    ...
    if (SCICARDDN == (aSCI_BaseTable[pdev->terminal_ch]->SCI_IIR & SCICARDDN))
    {
        ...
        if (SCICARDIN == (aSCI_BaseTable[pdev->terminal_ch]->SCI_IIR & SCICARDIN))
        {
            ...
            //while (0 == (aSCI_BaseTable[pdev->terminal_ch]->SCI_IIR & SCICARDOUT));
            //SYSCTRL->PHER_CTRL |= BIT(16);
            //while (0 == (aSCI_BaseTable[pdev->terminal_ch]->SCI_IIR & SCICARDIN));
            //SYSCTRL->SCI_GLF = (SYSCTRL->SCI_GLF & ~0xFFFFF) | (SYSCTRL->PCLK_1MS_VAL >> 2);
            //Used to avoid overload.
            ...
            u32TranStep = 1;
            if (u32TranStep == grecvErro[u32ErrorStep].u32Step)
            {
                u32ErrorStep++;
                scs_queue[pdev->terminal_ch].sf = grecvErro[u32ErrorStep].u32ErroNo;
                scs_queue[pdev->terminal_ch].pf = grecvErro[u32ErrorStep].u32Posi;
            }
...
int emv_hard_power_pump( struct emv_core *pdev )
{
    ...
    //Set working voltage.
    SYSCTRL->SCI_GLF = (SYSCTRL->SCI_GLF & ~(0x03 << 28)) | ((u32Tmp & 0x03) << 28);
    //printf("GLF %08x\n", SYSCTRL->SCI_GLF);
    if (SCICARDDN != (aSCI_BaseTable[pdev->terminal_ch]->SCI_IIR & SCICARDDN))
    {
...
{
    uint16_t u16Recv;
    int32_t s32Ip = 0, s32ErrNo = ICC_ERR_OCCUPY;
    SCI_TypeDef * pSCI_Base;
    SCI_EMV_PARAM volatile * pSCI_Param;
        ...
        else
        {
            ...
            else if (pSCI_Base->SCI_IIR & (SCIRXTIDE | SCIRTOUT))
            {
                while (pSCI_Base->SCI_RXCOUNT)
                {
                    ...
                    if (0 != (u16Recv & 0x100))
                    { 
                        pSCI_Base->SCI_BLKTIME = pSCI_Base->SCI_CHTIME = gu32Di * 960;
...
void outputQueue(char *pStr, uint32_t u32Slot)
{
    ...
    DBG_PRINT(pStr);
    DBG_PRINT("[%d]:\t",u32TranStep);
    while (i < scs_queue[u32Slot].ip)
    {
...
int emv_hard_xmit(struct emv_core *pdev)
{
    ...
    u32Tmp = SCICHTOUT | SCIBLKTOUT | SCITXERR;
    ...
    if (u32TranStep == grecvErro[u32ErrorStep].u32Step)
    {
        scs_queue[pdev->terminal_ch].sf = grecvErro[u32ErrorStep].u32ErroNo;
        pdev->queue->ip = 0x107;
        memset(pdev->queue->qbuf + grecvErro[u32ErrorStep].u32Posi, 0, pdev->queue->ip - grecvErro[u32ErrorStep].u32Posi);
        scs_queue[pdev->terminal_ch].pf = grecvErro[u32ErrorStep].u32Posi;
        u32ErrorStep++;
    }
