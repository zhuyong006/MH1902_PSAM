...
#define DMA_CFG_DEST_PER_Mask								(0x03U<<DMA_CFG_DEST_PER_Pos)//need write current channel num

#define DMA_CFG_SRC_PER_Pos									(7)
#define DMA_CFG_SRC_PER_Mask								(0x03U<<DMA_CFG_SRC_PER_Pos)//need write current channel num

extern void SYSCTRL_DMA_ChannelConfig(DMA_TypeDef* DMA_Channelx, uint32_t DMA_Peripheral, uint32_t DMA_DIR);
...
{
	uint32_t retChannelxBit = 0;
	
	if (DMA_Channelx == DMA_Channel_0)
	{
		retChannelxBit = DMA_CHANNEL_0_BIT;
	}
	else if (DMA_Channelx == DMA_Channel_1)
	{
		retChannelxBit = DMA_CHANNEL_1_BIT;
	}
	else if (DMA_Channelx == DMA_Channel_2)
	{
		retChannelxBit = DMA_CHANNEL_2_BIT;
	}
	else if (DMA_Channelx == DMA_Channel_3)
	{
		retChannelxBit = DMA_CHANNEL_3_BIT;
	}

	return retChannelxBit;
}
...
void DMA_Init(DMA_TypeDef *DMA_Channelx, DMA_InitTypeDef *DMA_InitStruct)
{
	...
	uint32_t tmpChannelxBit = 0;
	DMA_PeripheralInfo_Def DMA_SRC, DMA_DST;
	...
	// Interrupt Clear registers: ClearTfr, ClearBlock, ClearSrcTran, ClearDstTran, and ClearErr.
...
// 		return;
// 	
// 	DMA_Channelx->CFG_H &= ~(DMA_CFG_DEST_PER_Mask | DMA_CFG_SRC_PER_Mask);
// 	DMA_Channelx->CFG_H |= (Num_channel << DMA_CFG_DEST_PER_Pos);
// 	DMA_Channelx->CFG_H |= (Num_channel << DMA_CFG_SRC_PER_Pos);
// 	
// 	SYSCTRL->PHRIPHERAL0 &= ~(SYSCTRL_PHRIPHERAL0_DMA_CH0_IF_Mask << (Num_channel * 4));
// 	SYSCTRL->PHRIPHERAL0 |= (HardwareHandshake_Peripheral <<(Num_channel * 4));
}
...
ITStatus DMA_GetITStatus(DMA_TypeDef* DMA_Channelx,uint32_t DMA_IT)
{
	...
	if((DMA_IT_Status & tmpChannelxBit) != RESET)
	{
		bitStatus = SET;
	}
	else
...
FlagStatus DMA_GetRawStatus(DMA_TypeDef* DMA_Channelx,uint32_t DMA_IT)
{
	...
	if(DMA_Raw_Status & tmpChannelxBit)
	{
		bitStatus = SET;
	}
	else
...
void DMA_ChannelConfig(DMA_TypeDef* DMA_Channelx, uint32_t DMA_Peripheral, uint32_t DMA_DIR)
{
	SYSCTRL_DMA_ChannelConfig(DMA_Channelx, DMA_Peripheral, DMA_DIR);
}
